 ...
 *   - timer-based periodic RSS checks
 *   - soft-limit warning and hard-limit kill enforcement
 *   - ioctl dispatch for register/unregister
 *   - kernel linked list with mutex protection
 */
...
/* ==============================================================
 * TODO 1: Linked-list node struct for monitored containers.
 * ============================================================== */
 ...
 * (ioctl handlers and timer callback via workqueue) run in
 * process context. A spinlock would also work but a mutex allows
 * sleeping allocations (GFP_KERNEL) while the lock is held.
 * ============================================================== */
...
/* We use a workqueue to run the check in process context so we
 * can safely use the mutex and call get_task_mm(). */
static struct work_struct check_work;
...
/* ---------------------------------------------------------------
 * Work function: runs the periodic check in process context.
 * --------------------------------------------------------------- */
static void check_monitored(struct work_struct *w)
{
    ...
    list_for_each_entry_safe(entry, tmp, &monitored_list, list) {
        ...
        if (rss < 0) {
            printk(KERN_INFO
                   "[container_monitor] Removing exited container=%s pid=%d\n",
        ...
        /* Hard limit check (checked first so we kill immediately) */
...
static int __init monitor_init(void)
{
    ...
    INIT_WORK(&check_work, check_monitored);
